...
| D24 | Live log                    | **Live log from the files** (2026-09-14): capability `logs.stream` from the Files connector for games with a log file (`logs/latest.log`), followed by polling its size (folder 1 s, SFTP 2 s) while somebody watches; last 1000 lines in memory. Modules get `ctx.logs` and **server-sent events** (`ctx.http.serverEvents`: keep-alive pings, `X-Accel-Buffering: no`, streams end after 15 minutes and resume with `Last-Event-ID`). The Console shows it with the new permission `console.read` (owner, admin, moderator)                                                                                                                                                          | SSE needs nothing in CapRover or other proxies and only goes one way, which is all a log needs. Viewers do not see the log because it holds the chat and the IP addresses of players                                                         |
| D25 | File scopes and whitelist   | **Modules declare the files they use** (2026-09-14): `files: { read, write }` in the manifest, with `*` within a name and `**` across folders; the core refuses other paths in `ctx.files` (403 `file_out_of_scope`) and shows only declared files in listings. Capabilities stay technical (`files.read`, `files.write`). Players keeps the whitelist in `whitelist.json` when the files are writable (offline UUIDs from the names, `whitelist reload` over RCON, else at the next start), uses RCON otherwise, and shows the list, its state and a UUID doctor from the files when they are readable. Outpost never calls the Mojang API: online-mode servers add players over RCON | Finer capabilities bound to games were considered: they would multiply with every game and module and protect no more than paths per module do. Scopes make the file access of every module visible and limit what a faulty module can break |
| D26 | Player details              | **A separate module reads the player files** (2026-09-14): inventory, armor, ender chest, contents of shulker boxes and bundles, vitals, statistics and advancements from `<world>/players/*` (26.x) or `playerdata`, `stats` and `advancements`; files only, no RCON, so players online can be one autosave behind; the location behind its own permission (owner, admin). Item icons and names come from the official client of the world's version, downloaded from Mojang by an owner who accepts the EULA (as BlueMap does), reduced to icons (texture layers with tints, or block cubes) and English and Russian names, kept in the module's table and served by Outpost         | Owner's choices: a separate module; everything but containers around a point; files only; icons from Mojang rather than a third-party CDN (privacy, CSP, availability). Textures of the game must not be shipped in the MIT repository       |
...
Built in stage 13 (D25) as follows. With writable files the whitelist of an offline-mode server is
edited in `whitelist.json` as above; without RCON the server loads it when it starts next.
Online-mode servers add players over RCON, because Outpost does not call the Mojang API, and
remove them in the file. With read-only files the list and the doctor come from the file and
changes go over RCON. The mode is set by hand, else detected from the players online, else read
from `online-mode`. The doctor's fix is for offline-mode servers only.
...
| 12  | **Live log**                    | D24: `logs.stream` from the Files connector, log hub (tail, rotation, partial lines, idle close), `readRange` for both file sources, `ctx.logs`, server-sent event streams for modules, Console with the live log (`console.read`, levels coloured, follows the end unless scrolled up); command history per user and server in the database (last 100, history menu and arrow keys)                                                    | Tests: hub on a real file, SSE stream with resume, permissions                                                                                | M    |
| 13  | **Whitelist via files**         | D25: file scopes of modules in the manifest, enforced by the core; Players: whitelist in `whitelist.json` (offline UUIDs in the spelling of known players, reload over RCON or at the next start), mode also from `online-mode` and whitelist state from `white-list` of `server.properties`, UUID doctor with a fix for offline-mode servers, read-only files → changes over RCON; the Players tab also with the Files connector alone | Tests: scopes, offline UUIDs as Java computes them, every combination of RCON and files                                                       | M    |
| 14  | **Player details**              | D26: module `outpost.player-details` (NBT and ZIP readers on web streams, both world layouts, item formats since 1.13); tab with the players, inventory grids, item details with contents, ender chest, location, statistics and advancements; download of the icons with EULA consent, cubes and tints drawn in the browser; link from the Players card                                                                                | Tests: NBT, items of old and new formats, icons from client models, download from a fake Mojang; checked against a real 26.2 world and client | L    |
...
1. **Connectors** (D22; stages 10a and 10b add the Files connector): modules on top of the files —
   live log (done in stage 12), offline-mode whitelist and UUID doctor (done in stage 13), inventories, statistics and advancements of players (done in stage 14), `server.properties` view
