...
Conventional Commits + release-please · Dependabot · CodeQL · multi-arch image (amd64/arm64).
...
- **Supply chain**: lockfile, Dependabot, CodeQL, pinned action SHAs, SBOM + provenance for images.
...
- **CD**: push to `main` → image `ghcr.io/mobixon/outpost:edge`; release-please opens release PRs;
  tag `vX.Y.Z` → images `X.Y.Z`, `X.Y`, `latest` (amd64 + arm64), GitHub Release with changelog.
